...
Опционально: категория `Продвижение`, уровень `Только чтение`. Без неё финансовый отчёт работает, но рекламные расходы и ДРР не показываются.

Категория Analytics для текущего MVP не требуется. Сервис не загружает кампании, ключевые запросы или поисковую аналитику: из Promotion API используются только фактические расходы по артикулам.
