...
## Обновление: расходы на продвижение и детализация артикула

По согласованному объёму сервис получает из WB Promotion API только фактические
расходы, группирует их по `nmId` и дате и рассчитывает общий и поартикульный ДРР.
Аналитика кампаний, ключевых запросов, поисковых позиций, аномалии, отдельный рекламный
модуль и калькулятор unit-экономики не добавлялись. СПП нигде не показывается и не
участвует в формулах.

Расходы на продвижение сохраняются в `PromotionSpendDaily`; синхронизация использует
DB lock/cooldown через `WbSyncState`. Отсутствие категории `Продвижение` у WB-токена
не ломает Finance: отчёт остаётся доступен, а UI показывает, что для ДРР нет доступа.
Дашборд и карточка артикула теперь показывают удержания WB и их долю, фактическую
комиссию/вознаграждение, логистику на проданную единицу, выкуп, расходы на продвижение
и ДРР. В карточке артикула есть движения по дням, разрез по размерам и копирование
артикула.
...
версию `d2bf33fbd1e1`; Prisma подключился к существующей Neon PostgreSQL и подтвердил, что
...
внутри Telegram. Последний commit `d2bf33f` меняет только handoff-документы; код приложения
совпадает с проверенным product commit `485d65f`.
