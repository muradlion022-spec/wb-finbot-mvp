...
**DEPLOYED WITH LIMITATIONS.** Актуальная версия развёрнута в существующий Vercel
project `wb-finbot-mvp` на `https://wb-finbot-mvp.vercel.app`. Production health сообщает
версию `5f166b9a650d`; Prisma подключился к существующей Neon PostgreSQL и подтвердил, что
...
открылся и повторно прочитался из сохранённого состояния. Не выполнена только визуальная
приёмка пользователем внутри Telegram. GitHub `main` пока не синхронизирован: текущая
интеграция имеет read-only доступ, а локальные HTTPS credentials отсутствуют. Рабочий
production был безопасно обновлён прямым deploy в тот же Vercel project.
...
- GitHub `main` необходимо синхронизировать после восстановления write-аутентификации.
  Force push запрещён; текущий production deployment удалять нельзя.
